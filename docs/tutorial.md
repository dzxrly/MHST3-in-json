...
- `-i`/`--input-dir`：解包后的数据根目录或单个输入文件；
- `-s`/`--schema-path`/`--schema-dir`：RE_RSZ模板JSON文件路径，必须是具体文件，不能传目录；
- `-o`/`--output-dir`：JSON输出目录；
- `-p`/`--il2cpp-dump-path`：`il2cpp_dump.json`路径，导出时必填；
- `-d`/`--tree-depth`：对象树展开深度，默认`auto`；
- `-x`/`--exclude-regex`：排除相对路径的正则，可重复传多次；
- `--user-magic`：USR文件magic，默认`0x00525355`；
- `--rsz-magic`：RSZ块magic，默认`0x005A5352`。

### 2.4 将JSON封回`.user.3`

如果你修改了导出的JSON，可以用`pack`命令重新封包：
...
其中`-p`在封包时是可选的，但如果JSON中使用了枚举成员名，建议传入`il2cpp_dump.json`，便于枚举名反查。
...
如果想在找到指定`.user.3`后直接修改JSON并自动封回，可以使用callback工作流：
...
def edit_data(data, source_path: Path):
    root = data[0]["app.user_data.OtomonBasicParam"]
    root["_BodyScale"] = 1.0
    return data

converter.patch_directory(
...
callback可以返回新的JSON对象，也可以原地修改`data`后返回`None`。程序会自动把结果封成`.user.3`并写入输出目录。
